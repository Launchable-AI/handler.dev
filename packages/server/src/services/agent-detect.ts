...
// Ends with 'kill 0' to terminate background processes started by .bashrc (e.g. Handler status watcher)
// that would otherwise keep the SSH session open. Uses spawnSync since kill 0 gives non-zero exit.
...
).join('; ') + '; echo "__AGENT_DETECT_DONE__"; kill 0 2>/dev/null';
...
export async function detectAgentsViaSsh(
  ...
  try {
    ...
    if (!output.includes('__AGENT_DETECT_DONE__')) {
      return AGENTS.map(a => ({ id: a.id, name: a.name, installed: false, running: false }));
    }
    const agents = parseDetectionOutput(output);
    cache.set(sandboxId, { agents, timestamp: Date.now() });
